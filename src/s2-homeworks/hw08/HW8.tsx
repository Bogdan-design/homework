...
const HW8 = () => {
    ...
    const sortUp = () => {
        setPeople(
            homeWorkReducer(initialPeople, sortAC('up'))
        ) // в алфавитном порядке a.name > b.name
        setCurrentSort('up')
    }

    const sortDown = () => {
        setPeople(homeWorkReducer(initialPeople, sortAC('down'))) // в обратном порядке a.name < b.name}
        setCurrentSort('down')
    }
    const check18 = () => {
        setPeople(
            homeWorkReducer(initialPeople, check18AC(18))
        ) // совершеннолетние
        setCurrentSort('18')
    }
